...
- 2026-06-25 纯 Rust bridge 最终化收口：补齐官方 hook 事件安装范围，修正 Notification/question 为 attention-only 语义，避免 HUD 落盘或伪装注入 `answerText`；Desktop/Mobile pending intent resolve 会在入口拒绝过期 request，legacy pending audit 只写 `intentRef` hash；新增 `npm run verify:r6` 一键串联最终化全量验证。已完整通过 Phase R6 全量验证并重新打包安装；当前 bridge 为 `hud-bridge-34557e85fcb6.exe`，NSIS SHA256 为 `D0AB00B717058BB511ED86D58CDA5FB0E8A2E66A191F1A86ACD7FFF67B43410B`。随后补充修正 Android debug APK 打包语义：原 `build:android` 被 Gradle up-to-date 跳过导致 APK 时间戳未刷新；已新增 `build:android:fresh` 并强制 clean+assembleDebug，最新 `app-debug.apk` 时间戳为 `2026-06-25 09:45:26`，SHA256 为 `ED73E422EFF511A96BD49769FC3237328E2CACB98B7E4B8689864BB98666B375`。
- 2026-06-25 Desktop/Mobile 会话状态二次修复：原 activity 语义把 `UserPromptSubmit` 直接作为 running，且 Mobile capsule 把 `active` 映射成 running；同时缺少 `MessageDisplay` hook，普通 assistant streaming 会话没有 tool/agent running count 时可能显示 idle。已补齐 `MessageDisplay`、`PostToolUseFailure`、`PostToolBatch`、`SubagentStart/Stop`、`PostCompact` hook，并将 running 语义收敛为 MessageDisplay/工具/子代理/compact 等真实运行边界，UserPromptSubmit 改为 active；Mobile snapshot 改为派生 activity 且 active 不再强制 running。已重新打包安装 bridge `hud-bridge-2c249935189d.exe`，NSIS SHA256 为 `9412CA5E82CBCE4BAFB181710488FE57766062EC7C6C319E3F9190C99A1CFE2C`，Android debug APK SHA256 为 `35A22F30B4BDF56ABF2F8AB82096491AAE9985674270233CC4FF86B4E792DFD4`。
