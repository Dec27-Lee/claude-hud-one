...
test.describe('Claude HUD One UI smoke', () => {
  ...
  test('captures settings window route', async ({ page }) => {
    ...
    await expect(page.getByRole('tab', { name: '移动 HUD' })).toBeVisible()
    ...
    await page.getByRole('tab', { name: '移动 HUD' }).click()
    await expect(page.getByRole('heading', { name: 'Mobile HUD', exact: true })).toBeVisible()
    await expect(page.getByText('一期只读', { exact: false })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'PC Mobile HUD 服务' })).toBeVisible()
    await expect(page.getByRole('heading', { name: '配对与设备' })).toBeVisible()
    await expect(page.getByText('完整 token', { exact: false })).toBeVisible()
    await screenshot(page, '05-mobile-hud-settings')
